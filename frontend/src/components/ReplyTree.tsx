...
const ReplyNode = ({ reply, postId, depth = 0 }: { reply: any; postId: string; depth?: number }) => {
 ...
 return (
 <div className={`mt-4 ${depth > 0 ? 'ml-6 pl-4 border-l-2 border-gray-100 ' : ''}`}>
      <div className="flex items-start gap-3">
        <UserLink userId={reply.authorId || reply.author.id}>
          <img 
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <UserLink userId={reply.authorId || reply.author.id}>
              <span className="font-semibold text-sm text-foreground hover:underline">{reply.author.name}</span>
            </UserLink>
            <span className="text-xs text-muted">{formatDistanceToNow(new Date(reply.createdAt))} ago</span>
          </div>
 ...
 className="flex-1 bg-gray-50 border border-gray-200 rounded-full px-4 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
 value={content}
...
export const ReplyTree = ({ replies, postId }: { replies: any[]; postId: string }) => {
 ...
 <h3 className="font-semibold text-lg mb-4 text-gray-900 ">Replies</h3>
 {tree.map(node => (
