...
export const Search = () => {
 ...
 (_err) => {
 toast.error('Failed to get location from browser. Please set your home location in your profile.');
 setIsNearbyMode(false);
 }
 );
 }
 }
 }, [isNearbyMode, profile, coords]);
 ...
 <div>
 <h3 className="font-bold text-xl mb-4 flex items-center gap-2"><Users className="w-5 h-5 text-indigo-500"/> Players</h3>
 <motion.div initial="hidden" animate="visible" variants={staggerVariants} className="grid grid-cols-1 md:grid-cols-3 gap-4">
 {searchResults.users.map((u: any) => (
 <MotionLink variants={itemVariants} to={`/users/${u.id}`} key={u.id} className="bg-white border rounded-xl p-4 hover:border-indigo-500 flex flex-col items-center text-center">
 <img src={u.profile?.avatarUrl || `https://ui-avatars.com/api/?name=${u.name}&background=random`} alt={u.name} className="w-16 h-16 rounded-full mb-3" />
 <h4 className="font-bold text-sm">{u.name}</h4>
 <p className="text-xs text-gray-500 mt-1">{u.profile?.sports?.slice(0, 2).join(', ') || 'Player'}</p>
 </MotionLink>
 ))}
 </motion.div>
 </div>
 ...
 {/* Search Header */}
 <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 md:p-10 mb-8 text-center relative overflow-hidden">
 {isAIMode && (
 ...
 className={`w-full pl-12 pr-4 py-4 rounded-full border-2 focus:outline-none bg-gray-50 transition-colors ${
 ...
 />
 <span className={`font-medium ${type === t ? 'text-gray-900 ' : 'text-gray-500'}`}>
