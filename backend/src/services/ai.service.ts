...
export class AIService {
  ...
  async moderateContent(text: string): Promise<{ isSpam: boolean; reason: string }> {
    if (!ai) return { isSpam: false, reason: 'AI disabled' };
  ...
  async detectFakeEvent(title: string, sport: string, cost: number): Promise<{ isFake: boolean; reason: string }> {
    if (!ai) return { isFake: false, reason: 'AI disabled' };
  ...
  async getRecommendations(userProfile: any, matches: any[]): Promise<any[]> {
    if (!ai || matches.length === 0) return [];
    ...
    try {
      ...
      return recommendations.map((rec: any) => ({
        ...
        reason: rec.reason
      })).filter((r: any) => r.match !== undefined);
